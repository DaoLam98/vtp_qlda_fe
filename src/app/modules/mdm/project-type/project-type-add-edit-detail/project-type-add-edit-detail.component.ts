...
import {FORM_CONFIG} from 'src/app/modules/mdm/project-type/project-type-search/project-type-search.config';
import { ProjectTypeModel } from 'src/app/modules/mdm/_models/project-type.model';

@Component({
  selector: 'app-project-type-add-edit-detail',
  standalone: false,
  templateUrl: './project-type-add-edit-detail.component.html',
})
export class ProjectTypeAddEditDetailComponent extends BaseAddEditComponent implements OnInit {

  moduleName = 'mdm.project-type';
  configForm: Config;
  model: ProjectTypeModel | null = null;
  isView = false;
  ...
  constructor(
    ...
    this.addEditForm = this.fb.group({
      code: ['', [Validators.maxLength(30), Validators.pattern(DEFAULT_REGEX)]],
  ...
  async ngOnInit() {
    ...
    if (this.isEdit) {
      this.model = await firstValueFrom(
        this.apiService.get<ProjectTypeModel>(`${environment.PATH_API_V1}/mdm/project-type/` + this.id, new HttpParams())
      );
  ...
  onUpdateStatus(status: 'approve' | 'reject') {
    const apiCall = this.apiService.post(`${environment.PATH_API_V1}/mdm/project-type/${this.id}/${status}`, '');
  ...
  onRedirect(item: any) {
    this.router.navigate([`/mdm/project-type/edit`, item]).then();
  }
  ...
  onSubmit() {
    ...
    const payload = new ProjectTypeModel(this.addEditForm);
    formData.append('body', this.utilsService.toBlobJon(payload));

    const apiCall = this.isEdit
      ? this.apiService.put(`${environment.PATH_API_V1}/mdm/project-type/${this.id}`, formData)
      : this.apiService.post(`${environment.PATH_API_V1}/mdm/project-type`, formData);
