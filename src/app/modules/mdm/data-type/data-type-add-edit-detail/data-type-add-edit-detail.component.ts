...
@Component({
  selector: 'app-data-type-add-edit-detail',
  standalone: false,
  templateUrl: './data-type-add-edit-detail.component.html'
})
export class DataTypeAddEditDetailComponent extends BaseAddEditComponent implements OnInit {
  moduleName: string = 'mdm.information-type';
  configForm: Config;
  model: DataTypeModel | null = null;
  ...
  async ngOnInit() {
    ...
    if(this.isEdit) {
      this.model = await firstValueFrom(
        this.apiService.get<DataTypeModel>(
    ...
    if(this.isView) {
      this.addEditForm.get('isPeriodControl')?.disable();
    }
  }
  ...
  onRedirect(item: any) {
    this.router.navigate([`/mdm/data-type/edit`, item]).then();
  }

  onSubmit() {
    const formData = new FormData();
    const payload = new DataTypeModel(this.addEditForm);
