...
@Component({
  selector: 'app-asset-group-add-edit-detail',
  standalone: false,
  templateUrl: './asset-group-add-edit-detail.component.html'
})
export class AssetGroupAddEditDetailComponent extends BaseAddEditComponent implements OnInit {
  moduleName: string = 'mdm.asset-group';
  configForm: Config;
  model: AssetGroupModel | null = null;
  ...
  async ngOnInit() {
    ...
    if(this.isEdit) {
      this.model = await firstValueFrom(
        this.apiService.get<AssetGroupModel>(
  ...
  onRedirect(item: any) {
    this.router.navigate([`/mdm/asset-group/edit`, item]).then();
  }
