...
import { FORM_CONFIG } from './report-cumulative.config';

@Component({
  selector: 'app-report-cumulative',
  templateUrl: './report-cumulative.component.html',
  styleUrl: './report-cumulative.component.scss',
  standalone: false,
})
export class ReportCumulativeComponent implements OnChanges {
  ...
  constructor(
    ...
    this.columns = [
      ...
      },
      {
        columnDef: 'rate',
        header: 'valueGap',
        title: (e: ReportItemModel) => this.getValueGap(e),
        cell: (e: ReportItemModel) => this.getValueGap(e),
  ...
  convertField2HttpParamFn = (params: HttpParams) => {
    ...
    const endDateStr = this.formAdvanceSearch.value.endDate.toString();
    const formattedEndDate = `${endDateStr?.split(' ')[0]} 23:59:59.000Z`;
    params = params.set('reportType', 'CUMULATIVE').set('endDate', formattedEndDate);
  ...
  onDownload() {
    ...
    params = this.searchParams
      .set('templateCode', 'EXPORT_CUMULATIVE_REPORT')
  ...
  getValueGap(e: ReportItemModel) {
    return e.valueGap ? Utils.formatCurrency(e.valueGap) : '';
  }
}
